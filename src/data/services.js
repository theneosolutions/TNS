const Data = [
  {
    id: 1,
    name: "mobile-app",
    data: {
      name: "Mobile Development",
      section1: {
        content1:
          "Welcome to The Neo Solutions: Your Gateway to Seamless Mobile Experiences At Neo Solutions, we are your trusted partner in crafting cutting-edge mobile applications that redefine user experiences.",
        content2:
          "Our expertise lies in leveraging the power of React Native for front-end development and Java or Node.js for robust backends, ensuring that your app stands out in today's dynamic digital landscape.",
        list: [
          "API Development",
          "Cloud Migration",
          "TypeScript",
          "React Native",
          "Front End Development",
          "Fluter Framework",
        ],
        largeImage: "../../../public/assets/imgs/about/3/1.jpg",

        // largeImage: "../public/assets/imgs/about/3/1.jpg",
        smallImage: "",
      },
      section2: [
        {
          name: "Expertise",
          discription:
            "Our team comprises seasoned developers and designers with a proven track record in creating successful mobile applications.ess the power of Apple’s ecosystem with custom iOS apps designed to engage and delight your users.",
        },
        {
          name: "Custom Solutions",
          discription:
            "We understand that every project is unique. Our solutions are tailored to meet your specific business needs, ensuring a personalized and effective mobile app.",
        },
        {
          name: "Support & Maintenance",
          discription:
            "Our commitment extends beyond the development phase. We offer continuous support and maintenance services, ensuring your app remains up-to-date and performs optimally.",
        },
        {
          name: "Support & Maintenance",
          discription:
            "Our commitment extends beyond the development phase. We offer continuous support and maintenance services, ensuring your app remains up-to-date and performs optimally.",
        },
      ],
      section3: {
        heading:
          "We replace unreliable wirefreme and expensive agencies for one of the best organized layer.",
        image: "",
        content1:
          "Cross-Platform Excellence: Reach a wider audience with a single codebase. React Native allows us to develop mobile applications that seamlessly run on both iOS and Android platforms, saving you time and resources.",
        content2:
          "Native Performance: Experience the speed and responsiveness of native apps without compromising on development efficiency. React Native ensures a smooth and engaging user interface that meets the highest performance standards.",
      },
      section4: {
        heading: "Product making for friendly users",
        image: "",
        list: [
          {
            eventKey: 1,
            question: "Scalability",
            answer:
              "Java provides a solid foundation for scalable backends, ensuring your app can handle increasing user loads and data demands.",
          },
          {
            eventKey: 2,
            question: "Security",
            answer:
              "With Java's mature security features, your app and user data are protected against potential threats, building trust and confidence.",
          },
          {
            eventKey: 3,
            question: "Real-time Applications",
            answer:
              "Leverage the asynchronous, event-driven architecture of Node.js for real-time features, making your app ideal for chat applications, collaborative tools, and more.",
          },
          {
            eventKey: 4,
            question: "Fast Execution",
            answer:
              "Node.js's lightweight and efficient runtime ensures faster execution of code, optimizing the overall performance of your backend.",
          },
        ],
      },
      section5: {
        text: "WORK WITH US",
        heading: "We would love to hear more about your project",
      },
    },
  },
  {
    id: 2,
    name: "web-app",
    data: {
      name: "Website Development",
      section1: {
        content1:
          "Welcome to Neo Solutions: Elevating Digital Experiences with Next-Gen Web Apps At Neo Solutions, we are at the forefront of digital innovation, providing comprehensive web app development services that seamlessly blend cutting-edge technologies. ",
        content2:
          "With a focus on React, Next.js, and robust backend solutions in Java or Node.js, we empower businesses to thrive in the ever-evolving digital landscape.",
        list: [
          "API Development",
          "Cloud Migration",
          "TypeScript",
          "React & Next.Js",
          "Front End Development",
          "Backend Development ",
        ],
        largeImage: "../../../public/assets/imgs/about/3/1.jpg",

        // largeImage: "../public/assets/imgs/about/3/1.jpg",
        smallImage: "",
      },
      section2: [
        {
          name: "Reliability",
          discription:
            "Java is renowned for its stability and reliability, making it an ideal choice for robust backend development. Your web app will run smoothly, even under heavy loads and complex operations.",
        },
        {
          name: "Security",
          discription:
            "Benefit from Java's mature security features, ensuring that your application and user data are protected against potential threats.",
        },
        {
          name: "Scalability",
          discription:
            "Node.js's non-blocking, event-driven architecture makes it an excellent choice for scalable applications, ensuring your web app can handle increasing user demands.",
        },
        {
          name: "Speed",
          discription:
            "Leverage the fast execution of JavaScript in the server environment, providing a swift and responsive user experience for your web app.",
        },
      ],
      section3: {
        heading: "Why Choose Neo Solutions?",
        image: "",
        content1:
          "Innovative Solutions: Our team thrives on innovation, constantly exploring new technologies to provide web app solutions that are ahead of the curve.",
        content2:
          "Collaborative Approach: We believe in transparent and collaborative partnerships, keeping you involved at every stage of the development process to ensure the end product aligns with your vision.",
        content3:
          "Scalability & Future-Proofing: Our solutions are designed with scalability in mind, ensuring your web app can grow seamlessly with your business. We future-proof your digital presence with the latest technologies.",
      },
      section4: {
        heading: "Product making for friendly users",
        image: "",
        list: [
          {
            eventKey: 1,
            question: "Design should enrich our day",
            answer:
              "Our design services starts and ends with a best-in-class experience strategy that builds brands. Through a process of iteration and prototyping design interfaces that bring joy to people",
          },
          {
            eventKey: 2,
            question: "Bring their individual experience and creative",
            answer:
              "This is the second item's accordion body. It is hidden by default, until the collapse plugin adds the appropriate classes that we use to style each element. These classes control the overall appearance, as well as the showing and hiding via CSS transitions. You can modify any of this with custom CSS or overriding our default variables.",
          },
          {
            eventKey: 3,
            question: "Human centred design to challenges",
            answer:
              "Our design services starts and ends with a best-in-class experience strategy that builds brands. Through a process of iteration and prototyping design interfaces that bring joy to people",
          },
          {
            eventKey: 4,
            question: "Design should enrich our day",
            answer:
              "Our design services starts and ends with a best-in-class experience strategy that builds brands. Through a process of iteration and prototyping design interfaces that bring joy to people",
          },
          {
            eventKey: 5,
            question: "Developing core web applications",
            answer:
              "Our design services starts and ends with a best-in-class experience strategy that builds brands. Through a process of iteration and prototyping design interfaces that bring joy to people",
          },
        ],
      },
      section5: {
        text: "WORK WITH US",
        heading: "We would love to hear more about your project",
      },
    },
  },
  {
    id: 3,
    name: "saas",
    data: {
      name: "Saas Development",
      section1: {
        content1:
          "Every decision we make needs to answer the million-dollar question: hozzzzzzzw will this benefit our partner’s goal best? We work to develop solutions",
        content2:
          "We like to be different, in the same way that every mission is different. Every project is an opportunity to create result that will help you",
        list: [
          "API Development",
          "Cloud Migration",
          "JavaScript",
          "WordPress",
          "Front End Development",
          "Fluter Framework",
        ],
        largeImage: "../../../public/assets/imgs/about/3/1.jpg",

        // largeImage: "../public/assets/imgs/about/3/1.jpg",
        smallImage: "",
      },
      section2: [
        {
          name: "Planning and sketching",
          discription:
            "Modern and unique design practically point of view, it risks not meeting the huge expectations",
        },
        {
          name: "Team working",
          discription:
            "Modern and unique design practically point of view, it risks not meeting the huge expectations",
        },
        {
          name: "Flowchart and wireframe",
          discription:
            "Modern and unique design practically point of view, it risks not meeting the huge expectations",
        },
        {
          name: "User experience Testing",
          discription:
            "Modern and unique design practically point of view, it risks not meeting the huge expectations",
        },
      ],
      section3: {
        heading:
          "We replace unreliable wirefreme and expensive agencies for one of the best organized layer.",
        image: "",
        content1:
          "Receive your design within a few business days, and be updated on the process. Everything you need for a digitally driven brand. Defined proposition. Conceptual realisation. Logo, type, look, feel, tone, movement, content - we've got it covered.",
        content2:
          "Getting your brand message out there. We create dynamic campaign creative that engages audiences, wherever they are most talented. Bring your brand to life, communicate your value proposition with agile setup across creativity.",
      },
      section4: {
        heading: "Product making for friendly users",
        image: "",
        list: [
          {
            eventKey: 1,
            question: "Design should enrich our day",
            answer:
              "Our design services starts and ends with a best-in-class experience strategy that builds brands. Through a process of iteration and prototyping design interfaces that bring joy to people",
          },
          {
            eventKey: 2,
            question: "Bring their individual experience and creative",
            answer:
              "This is the second item's accordion body. It is hidden by default, until the collapse plugin adds the appropriate classes that we use to style each element. These classes control the overall appearance, as well as the showing and hiding via CSS transitions. You can modify any of this with custom CSS or overriding our default variables.",
          },
          {
            eventKey: 3,
            question: "Human centred design to challenges",
            answer:
              "Our design services starts and ends with a best-in-class experience strategy that builds brands. Through a process of iteration and prototyping design interfaces that bring joy to people",
          },
          {
            eventKey: 4,
            question: "Design should enrich our day",
            answer:
              "Our design services starts and ends with a best-in-class experience strategy that builds brands. Through a process of iteration and prototyping design interfaces that bring joy to people",
          },
          {
            eventKey: 5,
            question: "Developing core web applications",
            answer:
              "Our design services starts and ends with a best-in-class experience strategy that builds brands. Through a process of iteration and prototyping design interfaces that bring joy to people",
          },
        ],
      },
      section5: {
        text: "WORK WITH US",
        heading: "We would love to hear more about your project",
      },
    },
  },
  {
    id: 4,
    name: "software",
    data: {
      name: "Software Development",
      section1: {
        content1:
          "Every decision we make needs to answer the million-dollar question: hozzzzzzzw will this benefit our partner’s goal best? We work to develop solutions",
        content2:
          "We like to be different, in the same way that every mission is different. Every project is an opportunity to create result that will help you",
        list: [
          "API Development",
          "Cloud Migration",
          "JavaScript",
          "WordPress",
          "Front End Development",
          "Fluter Framework",
        ],
        largeImage: "../../../public/assets/imgs/about/3/1.jpg",

        // largeImage: "../public/assets/imgs/about/3/1.jpg",
        smallImage: "",
      },
      section2: [
        {
          name: "Planning and sketching",
          discription:
            "Modern and unique design practically point of view, it risks not meeting the huge expectations",
        },
        {
          name: "Team working",
          discription:
            "Modern and unique design practically point of view, it risks not meeting the huge expectations",
        },
        {
          name: "Flowchart and wireframe",
          discription:
            "Modern and unique design practically point of view, it risks not meeting the huge expectations",
        },
        {
          name: "User experience Testing",
          discription:
            "Modern and unique design practically point of view, it risks not meeting the huge expectations",
        },
      ],
      section3: {
        heading:
          "We replace unreliable wirefreme and expensive agencies for one of the best organized layer.",
        image: "",
        content1:
          "Receive your design within a few business days, and be updated on the process. Everything you need for a digitally driven brand. Defined proposition. Conceptual realisation. Logo, type, look, feel, tone, movement, content - we've got it covered.",
        content2:
          "Getting your brand message out there. We create dynamic campaign creative that engages audiences, wherever they are most talented. Bring your brand to life, communicate your value proposition with agile setup across creativity.",
      },
      section4: {
        heading: "Product making for friendly users",
        image: "",
        list: [
          {
            eventKey: 1,
            question: "Design should enrich our day",
            answer:
              "Our design services starts and ends with a best-in-class experience strategy that builds brands. Through a process of iteration and prototyping design interfaces that bring joy to people",
          },
          {
            eventKey: 2,
            question: "Bring their individual experience and creative",
            answer:
              "This is the second item's accordion body. It is hidden by default, until the collapse plugin adds the appropriate classes that we use to style each element. These classes control the overall appearance, as well as the showing and hiding via CSS transitions. You can modify any of this with custom CSS or overriding our default variables.",
          },
          {
            eventKey: 3,
            question: "Human centred design to challenges",
            answer:
              "Our design services starts and ends with a best-in-class experience strategy that builds brands. Through a process of iteration and prototyping design interfaces that bring joy to people",
          },
          {
            eventKey: 4,
            question: "Design should enrich our day",
            answer:
              "Our design services starts and ends with a best-in-class experience strategy that builds brands. Through a process of iteration and prototyping design interfaces that bring joy to people",
          },
          {
            eventKey: 5,
            question: "Developing core web applications",
            answer:
              "Our design services starts and ends with a best-in-class experience strategy that builds brands. Through a process of iteration and prototyping design interfaces that bring joy to people",
          },
        ],
      },
      section5: {
        text: "WORK WITH US",
        heading: "We would love to hear more about your project",
      },
    },
  },
  {
    id: 5,
    name: "microservices",
    data: {
      name: "Microservices",
      section1: {
        content1:
          "Every decision we make needs to answer the million-dollar question: hozzzzzzzw will this benefit our partner’s goal best? We work to develop solutions",
        content2:
          "We like to be different, in the same way that every mission is different. Every project is an opportunity to create result that will help you",
        list: [
          "API Development",
          "Cloud Migration",
          "JavaScript",
          "WordPress",
          "Front End Development",
          "Fluter Framework",
        ],
        largeImage: "../../../public/assets/imgs/about/3/1.jpg",

        // largeImage: "../public/assets/imgs/about/3/1.jpg",
        smallImage: "",
      },
      section2: [
        {
          name: "Planning and sketching",
          discription:
            "Modern and unique design practically point of view, it risks not meeting the huge expectations",
        },
        {
          name: "Team working",
          discription:
            "Modern and unique design practically point of view, it risks not meeting the huge expectations",
        },
        {
          name: "Flowchart and wireframe",
          discription:
            "Modern and unique design practically point of view, it risks not meeting the huge expectations",
        },
        {
          name: "User experience Testing",
          discription:
            "Modern and unique design practically point of view, it risks not meeting the huge expectations",
        },
      ],
      section3: {
        heading:
          "We replace unreliable wirefreme and expensive agencies for one of the best organized layer.",
        image: "",
        content1:
          "Receive your design within a few business days, and be updated on the process. Everything you need for a digitally driven brand. Defined proposition. Conceptual realisation. Logo, type, look, feel, tone, movement, content - we've got it covered.",
        content2:
          "Getting your brand message out there. We create dynamic campaign creative that engages audiences, wherever they are most talented. Bring your brand to life, communicate your value proposition with agile setup across creativity.",
      },
      section4: {
        heading: "Product making for friendly users",
        image: "",
        list: [
          {
            eventKey: 1,
            question: "Design should enrich our day",
            answer:
              "Our design services starts and ends with a best-in-class experience strategy that builds brands. Through a process of iteration and prototyping design interfaces that bring joy to people",
          },
          {
            eventKey: 2,
            question: "Bring their individual experience and creative",
            answer:
              "This is the second item's accordion body. It is hidden by default, until the collapse plugin adds the appropriate classes that we use to style each element. These classes control the overall appearance, as well as the showing and hiding via CSS transitions. You can modify any of this with custom CSS or overriding our default variables.",
          },
          {
            eventKey: 3,
            question: "Human centred design to challenges",
            answer:
              "Our design services starts and ends with a best-in-class experience strategy that builds brands. Through a process of iteration and prototyping design interfaces that bring joy to people",
          },
          {
            eventKey: 4,
            question: "Design should enrich our day",
            answer:
              "Our design services starts and ends with a best-in-class experience strategy that builds brands. Through a process of iteration and prototyping design interfaces that bring joy to people",
          },
          {
            eventKey: 5,
            question: "Developing core web applications",
            answer:
              "Our design services starts and ends with a best-in-class experience strategy that builds brands. Through a process of iteration and prototyping design interfaces that bring joy to people",
          },
        ],
      },
      section5: {
        text: "WORK WITH US",
        heading: "We would love to hear more about your project",
      },
    },
  },
  {
    id: 6,
    name: "cyber-security",
    data: {
      name: "Cyber Security",
      section1: {
        content1:
          "Every decision we make needs to answer the million-dollar question: hozzzzzzzw will this benefit our partner’s goal best? We work to develop solutions",
        content2:
          "We like to be different, in the same way that every mission is different. Every project is an opportunity to create result that will help you",
        list: [
          "API Development",
          "Cloud Migration",
          "JavaScript",
          "WordPress",
          "Front End Development",
          "Fluter Framework",
        ],
        largeImage: "../../../public/assets/imgs/about/3/1.jpg",

        // largeImage: "../public/assets/imgs/about/3/1.jpg",
        smallImage: "",
      },
      section2: [
        {
          name: "Planning and sketching",
          discription:
            "Modern and unique design practically point of view, it risks not meeting the huge expectations",
        },
        {
          name: "Team working",
          discription:
            "Modern and unique design practically point of view, it risks not meeting the huge expectations",
        },
        {
          name: "Flowchart and wireframe",
          discription:
            "Modern and unique design practically point of view, it risks not meeting the huge expectations",
        },
        {
          name: "User experience Testing",
          discription:
            "Modern and unique design practically point of view, it risks not meeting the huge expectations",
        },
      ],
      section3: {
        heading:
          "We replace unreliable wirefreme and expensive agencies for one of the best organized layer.",
        image: "",
        content1:
          "Receive your design within a few business days, and be updated on the process. Everything you need for a digitally driven brand. Defined proposition. Conceptual realisation. Logo, type, look, feel, tone, movement, content - we've got it covered.",
        content2:
          "Getting your brand message out there. We create dynamic campaign creative that engages audiences, wherever they are most talented. Bring your brand to life, communicate your value proposition with agile setup across creativity.",
      },
      section4: {
        heading: "Product making for friendly users",
        image: "",
        list: [
          {
            eventKey: 1,
            question: "Design should enrich our day",
            answer:
              "Our design services starts and ends with a best-in-class experience strategy that builds brands. Through a process of iteration and prototyping design interfaces that bring joy to people",
          },
          {
            eventKey: 2,
            question: "Bring their individual experience and creative",
            answer:
              "This is the second item's accordion body. It is hidden by default, until the collapse plugin adds the appropriate classes that we use to style each element. These classes control the overall appearance, as well as the showing and hiding via CSS transitions. You can modify any of this with custom CSS or overriding our default variables.",
          },
          {
            eventKey: 3,
            question: "Human centred design to challenges",
            answer:
              "Our design services starts and ends with a best-in-class experience strategy that builds brands. Through a process of iteration and prototyping design interfaces that bring joy to people",
          },
          {
            eventKey: 4,
            question: "Design should enrich our day",
            answer:
              "Our design services starts and ends with a best-in-class experience strategy that builds brands. Through a process of iteration and prototyping design interfaces that bring joy to people",
          },
          {
            eventKey: 5,
            question: "Developing core web applications",
            answer:
              "Our design services starts and ends with a best-in-class experience strategy that builds brands. Through a process of iteration and prototyping design interfaces that bring joy to people",
          },
        ],
      },
      section5: {
        text: "WORK WITH US",
        heading: "We would love to hear more about your project",
      },
    },
  },
  {
    id: 7,
    name: "dev-ops",
    data: {
      name: "DevOps",
      section1: {
        content1:
          "Every decision we make needs to answer the million-dollar question: hozzzzzzzw will this benefit our partner’s goal best? We work to develop solutions",
        content2:
          "We like to be different, in the same way that every mission is different. Every project is an opportunity to create result that will help you",
        list: [
          "API Development",
          "Cloud Migration",
          "JavaScript",
          "WordPress",
          "Front End Development",
          "Fluter Framework",
        ],
        largeImage: "../../../public/assets/imgs/about/3/1.jpg",

        // largeImage: "../public/assets/imgs/about/3/1.jpg",
        smallImage: "",
      },
      section2: [
        {
          name: "Planning and sketching",
          discription:
            "Modern and unique design practically point of view, it risks not meeting the huge expectations",
        },
        {
          name: "Team working",
          discription:
            "Modern and unique design practically point of view, it risks not meeting the huge expectations",
        },
        {
          name: "Flowchart and wireframe",
          discription:
            "Modern and unique design practically point of view, it risks not meeting the huge expectations",
        },
        {
          name: "User experience Testing",
          discription:
            "Modern and unique design practically point of view, it risks not meeting the huge expectations",
        },
      ],
      section3: {
        heading:
          "We replace unreliable wirefreme and expensive agencies for one of the best organized layer.",
        image: "",
        content1:
          "Receive your design within a few business days, and be updated on the process. Everything you need for a digitally driven brand. Defined proposition. Conceptual realisation. Logo, type, look, feel, tone, movement, content - we've got it covered.",
        content2:
          "Getting your brand message out there. We create dynamic campaign creative that engages audiences, wherever they are most talented. Bring your brand to life, communicate your value proposition with agile setup across creativity.",
      },
      section4: {
        heading: "Product making for friendly users",
        image: "",
        list: [
          {
            eventKey: 1,
            question: "Design should enrich our day",
            answer:
              "Our design services starts and ends with a best-in-class experience strategy that builds brands. Through a process of iteration and prototyping design interfaces that bring joy to people",
          },
          {
            eventKey: 2,
            question: "Bring their individual experience and creative",
            answer:
              "This is the second item's accordion body. It is hidden by default, until the collapse plugin adds the appropriate classes that we use to style each element. These classes control the overall appearance, as well as the showing and hiding via CSS transitions. You can modify any of this with custom CSS or overriding our default variables.",
          },
          {
            eventKey: 3,
            question: "Human centred design to challenges",
            answer:
              "Our design services starts and ends with a best-in-class experience strategy that builds brands. Through a process of iteration and prototyping design interfaces that bring joy to people",
          },
          {
            eventKey: 4,
            question: "Design should enrich our day",
            answer:
              "Our design services starts and ends with a best-in-class experience strategy that builds brands. Through a process of iteration and prototyping design interfaces that bring joy to people",
          },
          {
            eventKey: 5,
            question: "Developing core web applications",
            answer:
              "Our design services starts and ends with a best-in-class experience strategy that builds brands. Through a process of iteration and prototyping design interfaces that bring joy to people",
          },
        ],
      },
      section5: {
        text: "WORK WITH US",
        heading: "We would love to hear more about your project",
      },
    },
  },
  {
    id: 8,
    name: "marketing",
    data: {
      name: "Marketing",
      section1: {
        content1:
          "Every decision we make needs to answer the million-dollar question: hozzzzzzzw will this benefit our partner’s goal best? We work to develop solutions",
        content2:
          "We like to be different, in the same way that every mission is different. Every project is an opportunity to create result that will help you",
        list: [
          "API Development",
          "Cloud Migration",
          "JavaScript",
          "WordPress",
          "Front End Development",
          "Fluter Framework",
        ],
        largeImage: "../../../public/assets/imgs/about/3/1.jpg",

        // largeImage: "../public/assets/imgs/about/3/1.jpg",
        smallImage: "",
      },
      section2: [
        {
          name: "Planning and sketching",
          discription:
            "Modern and unique design practically point of view, it risks not meeting the huge expectations",
        },
        {
          name: "Team working",
          discription:
            "Modern and unique design practically point of view, it risks not meeting the huge expectations",
        },
        {
          name: "Flowchart and wireframe",
          discription:
            "Modern and unique design practically point of view, it risks not meeting the huge expectations",
        },
        {
          name: "User experience Testing",
          discription:
            "Modern and unique design practically point of view, it risks not meeting the huge expectations",
        },
      ],
      section3: {
        heading:
          "We replace unreliable wirefreme and expensive agencies for one of the best organized layer.",
        image: "",
        content1:
          "Receive your design within a few business days, and be updated on the process. Everything you need for a digitally driven brand. Defined proposition. Conceptual realisation. Logo, type, look, feel, tone, movement, content - we've got it covered.",
        content2:
          "Getting your brand message out there. We create dynamic campaign creative that engages audiences, wherever they are most talented. Bring your brand to life, communicate your value proposition with agile setup across creativity.",
      },
      section4: {
        heading: "Product making for friendly users",
        image: "",
        list: [
          {
            eventKey: 1,
            question: "Design should enrich our day",
            answer:
              "Our design services starts and ends with a best-in-class experience strategy that builds brands. Through a process of iteration and prototyping design interfaces that bring joy to people",
          },
          {
            eventKey: 2,
            question: "Bring their individual experience and creative",
            answer:
              "This is the second item's accordion body. It is hidden by default, until the collapse plugin adds the appropriate classes that we use to style each element. These classes control the overall appearance, as well as the showing and hiding via CSS transitions. You can modify any of this with custom CSS or overriding our default variables.",
          },
          {
            eventKey: 3,
            question: "Human centred design to challenges",
            answer:
              "Our design services starts and ends with a best-in-class experience strategy that builds brands. Through a process of iteration and prototyping design interfaces that bring joy to people",
          },
          {
            eventKey: 4,
            question: "Design should enrich our day",
            answer:
              "Our design services starts and ends with a best-in-class experience strategy that builds brands. Through a process of iteration and prototyping design interfaces that bring joy to people",
          },
          {
            eventKey: 5,
            question: "Developing core web applications",
            answer:
              "Our design services starts and ends with a best-in-class experience strategy that builds brands. Through a process of iteration and prototyping design interfaces that bring joy to people",
          },
        ],
      },
      section5: {
        text: "WORK WITH US",
        heading: "We would love to hear more about your project",
      },
    },
  },
  {
    id: 9,
    name: "social-media-marketing",
    data: {
      name: "Social Media Marketing",
      section1: {
        content1:
          "Every decision we make needs to answer the million-dollar question: hozzzzzzzw will this benefit our partner’s goal best? We work to develop solutions",
        content2:
          "We like to be different, in the same way that every mission is different. Every project is an opportunity to create result that will help you",
        list: [
          "API Development",
          "Cloud Migration",
          "JavaScript",
          "WordPress",
          "Front End Development",
          "Fluter Framework",
        ],
        largeImage: "../../../public/assets/imgs/about/3/1.jpg",

        // largeImage: "../public/assets/imgs/about/3/1.jpg",
        smallImage: "",
      },
      section2: [
        {
          name: "Planning and sketching",
          discription:
            "Modern and unique design practically point of view, it risks not meeting the huge expectations",
        },
        {
          name: "Team working",
          discription:
            "Modern and unique design practically point of view, it risks not meeting the huge expectations",
        },
        {
          name: "Flowchart and wireframe",
          discription:
            "Modern and unique design practically point of view, it risks not meeting the huge expectations",
        },
        {
          name: "User experience Testing",
          discription:
            "Modern and unique design practically point of view, it risks not meeting the huge expectations",
        },
      ],
      section3: {
        heading:
          "We replace unreliable wirefreme and expensive agencies for one of the best organized layer.",
        image: "",
        content1:
          "Receive your design within a few business days, and be updated on the process. Everything you need for a digitally driven brand. Defined proposition. Conceptual realisation. Logo, type, look, feel, tone, movement, content - we've got it covered.",
        content2:
          "Getting your brand message out there. We create dynamic campaign creative that engages audiences, wherever they are most talented. Bring your brand to life, communicate your value proposition with agile setup across creativity.",
      },
      section4: {
        heading: "Product making for friendly users",
        image: "",
        list: [
          {
            eventKey: 1,
            question: "Design should enrich our day",
            answer:
              "Our design services starts and ends with a best-in-class experience strategy that builds brands. Through a process of iteration and prototyping design interfaces that bring joy to people",
          },
          {
            eventKey: 2,
            question: "Bring their individual experience and creative",
            answer:
              "This is the second item's accordion body. It is hidden by default, until the collapse plugin adds the appropriate classes that we use to style each element. These classes control the overall appearance, as well as the showing and hiding via CSS transitions. You can modify any of this with custom CSS or overriding our default variables.",
          },
          {
            eventKey: 3,
            question: "Human centred design to challenges",
            answer:
              "Our design services starts and ends with a best-in-class experience strategy that builds brands. Through a process of iteration and prototyping design interfaces that bring joy to people",
          },
          {
            eventKey: 4,
            question: "Design should enrich our day",
            answer:
              "Our design services starts and ends with a best-in-class experience strategy that builds brands. Through a process of iteration and prototyping design interfaces that bring joy to people",
          },
          {
            eventKey: 5,
            question: "Developing core web applications",
            answer:
              "Our design services starts and ends with a best-in-class experience strategy that builds brands. Through a process of iteration and prototyping design interfaces that bring joy to people",
          },
        ],
      },
      section5: {
        text: "WORK WITH US",
        heading: "We would love to hear more about your project",
      },
    },
  },

  {
    id: 10,
    name: "search-engine-optimization",
    data: {
      name: "Search Engine Optimization",
      section1: {
        content1:
          "Every decision we make needs to answer the million-dollar question: hozzzzzzzw will this benefit our partner’s goal best? We work to develop solutions",
        content2:
          "We like to be different, in the same way that every mission is different. Every project is an opportunity to create result that will help you",
        list: [
          "API Development",
          "Cloud Migration",
          "JavaScript",
          "WordPress",
          "Front End Development",
          "Fluter Framework",
        ],
        largeImage: "../../../public/assets/imgs/about/3/1.jpg",

        // largeImage: "../public/assets/imgs/about/3/1.jpg",
        smallImage: "",
      },
      section2: [
        {
          name: "Planning and sketching",
          discription:
            "Modern and unique design practically point of view, it risks not meeting the huge expectations",
        },
        {
          name: "Team working",
          discription:
            "Modern and unique design practically point of view, it risks not meeting the huge expectations",
        },
        {
          name: "Flowchart and wireframe",
          discription:
            "Modern and unique design practically point of view, it risks not meeting the huge expectations",
        },
        {
          name: "User experience Testing",
          discription:
            "Modern and unique design practically point of view, it risks not meeting the huge expectations",
        },
      ],
      section3: {
        heading:
          "We replace unreliable wirefreme and expensive agencies for one of the best organized layer.",
        image: "",
        content1:
          "Receive your design within a few business days, and be updated on the process. Everything you need for a digitally driven brand. Defined proposition. Conceptual realisation. Logo, type, look, feel, tone, movement, content - we've got it covered.",
        content2:
          "Getting your brand message out there. We create dynamic campaign creative that engages audiences, wherever they are most talented. Bring your brand to life, communicate your value proposition with agile setup across creativity.",
      },
      section4: {
        heading: "Product making for friendly users",
        image: "",
        list: [
          {
            eventKey: 1,
            question: "Design should enrich our day",
            answer:
              "Our design services starts and ends with a best-in-class experience strategy that builds brands. Through a process of iteration and prototyping design interfaces that bring joy to people",
          },
          {
            eventKey: 2,
            question: "Bring their individual experience and creative",
            answer:
              "This is the second item's accordion body. It is hidden by default, until the collapse plugin adds the appropriate classes that we use to style each element. These classes control the overall appearance, as well as the showing and hiding via CSS transitions. You can modify any of this with custom CSS or overriding our default variables.",
          },
          {
            eventKey: 3,
            question: "Human centred design to challenges",
            answer:
              "Our design services starts and ends with a best-in-class experience strategy that builds brands. Through a process of iteration and prototyping design interfaces that bring joy to people",
          },
          {
            eventKey: 4,
            question: "Design should enrich our day",
            answer:
              "Our design services starts and ends with a best-in-class experience strategy that builds brands. Through a process of iteration and prototyping design interfaces that bring joy to people",
          },
          {
            eventKey: 5,
            question: "Developing core web applications",
            answer:
              "Our design services starts and ends with a best-in-class experience strategy that builds brands. Through a process of iteration and prototyping design interfaces that bring joy to people",
          },
        ],
      },
      section5: {
        text: "WORK WITH US",
        heading: "We would love to hear more about your project",
      },
    },
  },
  {
    id: 11,
    name: "search-engine-marketing",
    data: {
      name: "Search Engine Marketing",
      section1: {
        content1:
          "Every decision we make needs to answer the million-dollar question: hozzzzzzzw will this benefit our partner’s goal best? We work to develop solutions",
        content2:
          "We like to be different, in the same way that every mission is different. Every project is an opportunity to create result that will help you",
        list: [
          "API Development",
          "Cloud Migration",
          "JavaScript",
          "WordPress",
          "Front End Development",
          "Fluter Framework",
        ],
        largeImage: "../../../public/assets/imgs/about/3/1.jpg",

        // largeImage: "../public/assets/imgs/about/3/1.jpg",
        smallImage: "",
      },
      section2: [
        {
          name: "Planning and sketching",
          discription:
            "Modern and unique design practically point of view, it risks not meeting the huge expectations",
        },
        {
          name: "Team working",
          discription:
            "Modern and unique design practically point of view, it risks not meeting the huge expectations",
        },
        {
          name: "Flowchart and wireframe",
          discription:
            "Modern and unique design practically point of view, it risks not meeting the huge expectations",
        },
        {
          name: "User experience Testing",
          discription:
            "Modern and unique design practically point of view, it risks not meeting the huge expectations",
        },
      ],
      section3: {
        heading:
          "We replace unreliable wirefreme and expensive agencies for one of the best organized layer.",
        image: "",
        content1:
          "Receive your design within a few business days, and be updated on the process. Everything you need for a digitally driven brand. Defined proposition. Conceptual realisation. Logo, type, look, feel, tone, movement, content - we've got it covered.",
        content2:
          "Getting your brand message out there. We create dynamic campaign creative that engages audiences, wherever they are most talented. Bring your brand to life, communicate your value proposition with agile setup across creativity.",
      },
      section4: {
        heading: "Product making for friendly users",
        image: "",
        list: [
          {
            eventKey: 1,
            question: "Design should enrich our day",
            answer:
              "Our design services starts and ends with a best-in-class experience strategy that builds brands. Through a process of iteration and prototyping design interfaces that bring joy to people",
          },
          {
            eventKey: 2,
            question: "Bring their individual experience and creative",
            answer:
              "This is the second item's accordion body. It is hidden by default, until the collapse plugin adds the appropriate classes that we use to style each element. These classes control the overall appearance, as well as the showing and hiding via CSS transitions. You can modify any of this with custom CSS or overriding our default variables.",
          },
          {
            eventKey: 3,
            question: "Human centred design to challenges",
            answer:
              "Our design services starts and ends with a best-in-class experience strategy that builds brands. Through a process of iteration and prototyping design interfaces that bring joy to people",
          },
          {
            eventKey: 4,
            question: "Design should enrich our day",
            answer:
              "Our design services starts and ends with a best-in-class experience strategy that builds brands. Through a process of iteration and prototyping design interfaces that bring joy to people",
          },
          {
            eventKey: 5,
            question: "Developing core web applications",
            answer:
              "Our design services starts and ends with a best-in-class experience strategy that builds brands. Through a process of iteration and prototyping design interfaces that bring joy to people",
          },
        ],
      },
      section5: {
        text: "WORK WITH US",
        heading: "We would love to hear more about your project",
      },
    },
  },
  {
    id: 12,
    name: "content-marketing",
    data: {
      name: "Content Marketing",
      section1: {
        content1:
          "Every decision we make needs to answer the million-dollar question: hozzzzzzzw will this benefit our partner’s goal best? We work to develop solutions",
        content2:
          "We like to be different, in the same way that every mission is different. Every project is an opportunity to create result that will help you",
        list: [
          "API Development",
          "Cloud Migration",
          "JavaScript",
          "WordPress",
          "Front End Development",
          "Fluter Framework",
        ],
        largeImage: "../../../public/assets/imgs/about/3/1.jpg",

        // largeImage: "../public/assets/imgs/about/3/1.jpg",
        smallImage: "",
      },
      section2: [
        {
          name: "Planning and sketching",
          discription:
            "Modern and unique design practically point of view, it risks not meeting the huge expectations",
        },
        {
          name: "Team working",
          discription:
            "Modern and unique design practically point of view, it risks not meeting the huge expectations",
        },
        {
          name: "Flowchart and wireframe",
          discription:
            "Modern and unique design practically point of view, it risks not meeting the huge expectations",
        },
        {
          name: "User experience Testing",
          discription:
            "Modern and unique design practically point of view, it risks not meeting the huge expectations",
        },
      ],
      section3: {
        heading:
          "We replace unreliable wirefreme and expensive agencies for one of the best organized layer.",
        image: "",
        content1:
          "Receive your design within a few business days, and be updated on the process. Everything you need for a digitally driven brand. Defined proposition. Conceptual realisation. Logo, type, look, feel, tone, movement, content - we've got it covered.",
        content2:
          "Getting your brand message out there. We create dynamic campaign creative that engages audiences, wherever they are most talented. Bring your brand to life, communicate your value proposition with agile setup across creativity.",
      },
      section4: {
        heading: "Product making for friendly users",
        image: "",
        list: [
          {
            eventKey: 1,
            question: "Design should enrich our day",
            answer:
              "Our design services starts and ends with a best-in-class experience strategy that builds brands. Through a process of iteration and prototyping design interfaces that bring joy to people",
          },
          {
            eventKey: 2,
            question: "Bring their individual experience and creative",
            answer:
              "This is the second item's accordion body. It is hidden by default, until the collapse plugin adds the appropriate classes that we use to style each element. These classes control the overall appearance, as well as the showing and hiding via CSS transitions. You can modify any of this with custom CSS or overriding our default variables.",
          },
          {
            eventKey: 3,
            question: "Human centred design to challenges",
            answer:
              "Our design services starts and ends with a best-in-class experience strategy that builds brands. Through a process of iteration and prototyping design interfaces that bring joy to people",
          },
          {
            eventKey: 4,
            question: "Design should enrich our day",
            answer:
              "Our design services starts and ends with a best-in-class experience strategy that builds brands. Through a process of iteration and prototyping design interfaces that bring joy to people",
          },
          {
            eventKey: 5,
            question: "Developing core web applications",
            answer:
              "Our design services starts and ends with a best-in-class experience strategy that builds brands. Through a process of iteration and prototyping design interfaces that bring joy to people",
          },
        ],
      },
      section5: {
        text: "WORK WITH US",
        heading: "We would love to hear more about your project",
      },
    },
  },
  {
    id: 13,
    name: "email-marketing",
    data: {
      name: "Email Marketing",
      section1: {
        content1:
          "Every decision we make needs to answer the million-dollar question: hozzzzzzzw will this benefit our partner’s goal best? We work to develop solutions",
        content2:
          "We like to be different, in the same way that every mission is different. Every project is an opportunity to create result that will help you",
        list: [
          "API Development",
          "Cloud Migration",
          "JavaScript",
          "WordPress",
          "Front End Development",
          "Fluter Framework",
        ],
        largeImage: "../../../public/assets/imgs/about/3/1.jpg",

        // largeImage: "../public/assets/imgs/about/3/1.jpg",
        smallImage: "",
      },
      section2: [
        {
          name: "Planning and sketching",
          discription:
            "Modern and unique design practically point of view, it risks not meeting the huge expectations",
        },
        {
          name: "Team working",
          discription:
            "Modern and unique design practically point of view, it risks not meeting the huge expectations",
        },
        {
          name: "Flowchart and wireframe",
          discription:
            "Modern and unique design practically point of view, it risks not meeting the huge expectations",
        },
        {
          name: "User experience Testing",
          discription:
            "Modern and unique design practically point of view, it risks not meeting the huge expectations",
        },
      ],
      section3: {
        heading:
          "We replace unreliable wirefreme and expensive agencies for one of the best organized layer.",
        image: "",
        content1:
          "Receive your design within a few business days, and be updated on the process. Everything you need for a digitally driven brand. Defined proposition. Conceptual realisation. Logo, type, look, feel, tone, movement, content - we've got it covered.",
        content2:
          "Getting your brand message out there. We create dynamic campaign creative that engages audiences, wherever they are most talented. Bring your brand to life, communicate your value proposition with agile setup across creativity.",
      },
      section4: {
        heading: "Product making for friendly users",
        image: "",
        list: [
          {
            eventKey: 1,
            question: "Design should enrich our day",
            answer:
              "Our design services starts and ends with a best-in-class experience strategy that builds brands. Through a process of iteration and prototyping design interfaces that bring joy to people",
          },
          {
            eventKey: 2,
            question: "Bring their individual experience and creative",
            answer:
              "This is the second item's accordion body. It is hidden by default, until the collapse plugin adds the appropriate classes that we use to style each element. These classes control the overall appearance, as well as the showing and hiding via CSS transitions. You can modify any of this with custom CSS or overriding our default variables.",
          },
          {
            eventKey: 3,
            question: "Human centred design to challenges",
            answer:
              "Our design services starts and ends with a best-in-class experience strategy that builds brands. Through a process of iteration and prototyping design interfaces that bring joy to people",
          },
          {
            eventKey: 4,
            question: "Design should enrich our day",
            answer:
              "Our design services starts and ends with a best-in-class experience strategy that builds brands. Through a process of iteration and prototyping design interfaces that bring joy to people",
          },
          {
            eventKey: 5,
            question: "Developing core web applications",
            answer:
              "Our design services starts and ends with a best-in-class experience strategy that builds brands. Through a process of iteration and prototyping design interfaces that bring joy to people",
          },
        ],
      },
      section5: {
        text: "WORK WITH US",
        heading: "We would love to hear more about your project",
      },
    },
  },

  {
    id: 14,
    name: "interactive-content-marketing",
    data: {
      name: "Interactive Content Marketing",
      section1: {
        content1:
          "Every decision we make needs to answer the million-dollar question: hozzzzzzzw will this benefit our partner’s goal best? We work to develop solutions",
        content2:
          "We like to be different, in the same way that every mission is different. Every project is an opportunity to create result that will help you",
        list: [
          "API Development",
          "Cloud Migration",
          "JavaScript",
          "WordPress",
          "Front End Development",
          "Fluter Framework",
        ],
        largeImage: "../../../public/assets/imgs/about/3/1.jpg",

        // largeImage: "../public/assets/imgs/about/3/1.jpg",
        smallImage: "",
      },
      section2: [
        {
          name: "Planning and sketching",
          discription:
            "Modern and unique design practically point of view, it risks not meeting the huge expectations",
        },
        {
          name: "Team working",
          discription:
            "Modern and unique design practically point of view, it risks not meeting the huge expectations",
        },
        {
          name: "Flowchart and wireframe",
          discription:
            "Modern and unique design practically point of view, it risks not meeting the huge expectations",
        },
        {
          name: "User experience Testing",
          discription:
            "Modern and unique design practically point of view, it risks not meeting the huge expectations",
        },
      ],
      section3: {
        heading:
          "We replace unreliable wirefreme and expensive agencies for one of the best organized layer.",
        image: "",
        content1:
          "Receive your design within a few business days, and be updated on the process. Everything you need for a digitally driven brand. Defined proposition. Conceptual realisation. Logo, type, look, feel, tone, movement, content - we've got it covered.",
        content2:
          "Getting your brand message out there. We create dynamic campaign creative that engages audiences, wherever they are most talented. Bring your brand to life, communicate your value proposition with agile setup across creativity.",
      },
      section4: {
        heading: "Product making for friendly users",
        image: "",
        list: [
          {
            eventKey: 1,
            question: "Design should enrich our day",
            answer:
              "Our design services starts and ends with a best-in-class experience strategy that builds brands. Through a process of iteration and prototyping design interfaces that bring joy to people",
          },
          {
            eventKey: 2,
            question: "Bring their individual experience and creative",
            answer:
              "This is the second item's accordion body. It is hidden by default, until the collapse plugin adds the appropriate classes that we use to style each element. These classes control the overall appearance, as well as the showing and hiding via CSS transitions. You can modify any of this with custom CSS or overriding our default variables.",
          },
          {
            eventKey: 3,
            question: "Human centred design to challenges",
            answer:
              "Our design services starts and ends with a best-in-class experience strategy that builds brands. Through a process of iteration and prototyping design interfaces that bring joy to people",
          },
          {
            eventKey: 4,
            question: "Design should enrich our day",
            answer:
              "Our design services starts and ends with a best-in-class experience strategy that builds brands. Through a process of iteration and prototyping design interfaces that bring joy to people",
          },
          {
            eventKey: 5,
            question: "Developing core web applications",
            answer:
              "Our design services starts and ends with a best-in-class experience strategy that builds brands. Through a process of iteration and prototyping design interfaces that bring joy to people",
          },
        ],
      },
      section5: {
        text: "WORK WITH US",
        heading: "We would love to hear more about your project",
      },
    },
  },
  {
    id: 15,
    name: "video-marketing",
    data: {
      name: "Video Marketing",
      section1: {
        content1:
          "Every decision we make needs to answer the million-dollar question: hozzzzzzzw will this benefit our partner’s goal best? We work to develop solutions",
        content2:
          "We like to be different, in the same way that every mission is different. Every project is an opportunity to create result that will help you",
        list: [
          "API Development",
          "Cloud Migration",
          "JavaScript",
          "WordPress",
          "Front End Development",
          "Fluter Framework",
        ],
        largeImage: "../../../public/assets/imgs/about/3/1.jpg",

        // largeImage: "../public/assets/imgs/about/3/1.jpg",
        smallImage: "",
      },
      section2: [
        {
          name: "Planning and sketching",
          discription:
            "Modern and unique design practically point of view, it risks not meeting the huge expectations",
        },
        {
          name: "Team working",
          discription:
            "Modern and unique design practically point of view, it risks not meeting the huge expectations",
        },
        {
          name: "Flowchart and wireframe",
          discription:
            "Modern and unique design practically point of view, it risks not meeting the huge expectations",
        },
        {
          name: "User experience Testing",
          discription:
            "Modern and unique design practically point of view, it risks not meeting the huge expectations",
        },
      ],
      section3: {
        heading:
          "We replace unreliable wirefreme and expensive agencies for one of the best organized layer.",
        image: "",
        content1:
          "Receive your design within a few business days, and be updated on the process. Everything you need for a digitally driven brand. Defined proposition. Conceptual realisation. Logo, type, look, feel, tone, movement, content - we've got it covered.",
        content2:
          "Getting your brand message out there. We create dynamic campaign creative that engages audiences, wherever they are most talented. Bring your brand to life, communicate your value proposition with agile setup across creativity.",
      },
      section4: {
        heading: "Product making for friendly users",
        image: "",
        list: [
          {
            eventKey: 1,
            question: "Design should enrich our day",
            answer:
              "Our design services starts and ends with a best-in-class experience strategy that builds brands. Through a process of iteration and prototyping design interfaces that bring joy to people",
          },
          {
            eventKey: 2,
            question: "Bring their individual experience and creative",
            answer:
              "This is the second item's accordion body. It is hidden by default, until the collapse plugin adds the appropriate classes that we use to style each element. These classes control the overall appearance, as well as the showing and hiding via CSS transitions. You can modify any of this with custom CSS or overriding our default variables.",
          },
          {
            eventKey: 3,
            question: "Human centred design to challenges",
            answer:
              "Our design services starts and ends with a best-in-class experience strategy that builds brands. Through a process of iteration and prototyping design interfaces that bring joy to people",
          },
          {
            eventKey: 4,
            question: "Design should enrich our day",
            answer:
              "Our design services starts and ends with a best-in-class experience strategy that builds brands. Through a process of iteration and prototyping design interfaces that bring joy to people",
          },
          {
            eventKey: 5,
            question: "Developing core web applications",
            answer:
              "Our design services starts and ends with a best-in-class experience strategy that builds brands. Through a process of iteration and prototyping design interfaces that bring joy to people",
          },
        ],
      },
      section5: {
        text: "WORK WITH US",
        heading: "We would love to hear more about your project",
      },
    },
  },
  {
    id: 16,
    name: "graphic-designing",
    data: {
      name: "Graphic Design",
      section1: {
        content1:
          "Every decision we make needs to answer the million-dollar question: hozzzzzzzw will this benefit our partner’s goal best? We work to develop solutions",
        content2:
          "We like to be different, in the same way that every mission is different. Every project is an opportunity to create result that will help you",
        list: [
          "API Development",
          "Cloud Migration",
          "JavaScript",
          "WordPress",
          "Front End Development",
          "Fluter Framework",
        ],
        largeImage: "../../../public/assets/imgs/about/3/1.jpg",

        // largeImage: "../public/assets/imgs/about/3/1.jpg",
        smallImage: "",
      },
      section2: [
        {
          name: "Planning and sketching",
          discription:
            "Modern and unique design practically point of view, it risks not meeting the huge expectations",
        },
        {
          name: "Team working",
          discription:
            "Modern and unique design practically point of view, it risks not meeting the huge expectations",
        },
        {
          name: "Flowchart and wireframe",
          discription:
            "Modern and unique design practically point of view, it risks not meeting the huge expectations",
        },
        {
          name: "User experience Testing",
          discription:
            "Modern and unique design practically point of view, it risks not meeting the huge expectations",
        },
      ],
      section3: {
        heading:
          "We replace unreliable wirefreme and expensive agencies for one of the best organized layer.",
        image: "",
        content1:
          "Receive your design within a few business days, and be updated on the process. Everything you need for a digitally driven brand. Defined proposition. Conceptual realisation. Logo, type, look, feel, tone, movement, content - we've got it covered.",
        content2:
          "Getting your brand message out there. We create dynamic campaign creative that engages audiences, wherever they are most talented. Bring your brand to life, communicate your value proposition with agile setup across creativity.",
      },
      section4: {
        heading: "Product making for friendly users",
        image: "",
        list: [
          {
            eventKey: 1,
            question: "Design should enrich our day",
            answer:
              "Our design services starts and ends with a best-in-class experience strategy that builds brands. Through a process of iteration and prototyping design interfaces that bring joy to people",
          },
          {
            eventKey: 2,
            question: "Bring their individual experience and creative",
            answer:
              "This is the second item's accordion body. It is hidden by default, until the collapse plugin adds the appropriate classes that we use to style each element. These classes control the overall appearance, as well as the showing and hiding via CSS transitions. You can modify any of this with custom CSS or overriding our default variables.",
          },
          {
            eventKey: 3,
            question: "Human centred design to challenges",
            answer:
              "Our design services starts and ends with a best-in-class experience strategy that builds brands. Through a process of iteration and prototyping design interfaces that bring joy to people",
          },
          {
            eventKey: 4,
            question: "Design should enrich our day",
            answer:
              "Our design services starts and ends with a best-in-class experience strategy that builds brands. Through a process of iteration and prototyping design interfaces that bring joy to people",
          },
          {
            eventKey: 5,
            question: "Developing core web applications",
            answer:
              "Our design services starts and ends with a best-in-class experience strategy that builds brands. Through a process of iteration and prototyping design interfaces that bring joy to people",
          },
        ],
      },
      section5: {
        text: "WORK WITH US",
        heading: "We would love to hear more about your project",
      },
    },
  },
  {
    id: 17,
    name: "video-design",
    data: {
      name: "Video Design",
      section1: {
        content1:
          "Every decision we make needs to answer the million-dollar question: hozzzzzzzw will this benefit our partner’s goal best? We work to develop solutions",
        content2:
          "We like to be different, in the same way that every mission is different. Every project is an opportunity to create result that will help you",
        list: [
          "API Development",
          "Cloud Migration",
          "JavaScript",
          "WordPress",
          "Front End Development",
          "Fluter Framework",
        ],
        largeImage: "../../../public/assets/imgs/about/3/1.jpg",

        // largeImage: "../public/assets/imgs/about/3/1.jpg",
        smallImage: "",
      },
      section2: [
        {
          name: "Planning and sketching",
          discription:
            "Modern and unique design practically point of view, it risks not meeting the huge expectations",
        },
        {
          name: "Team working",
          discription:
            "Modern and unique design practically point of view, it risks not meeting the huge expectations",
        },
        {
          name: "Flowchart and wireframe",
          discription:
            "Modern and unique design practically point of view, it risks not meeting the huge expectations",
        },
        {
          name: "User experience Testing",
          discription:
            "Modern and unique design practically point of view, it risks not meeting the huge expectations",
        },
      ],
      section3: {
        heading:
          "We replace unreliable wirefreme and expensive agencies for one of the best organized layer.",
        image: "",
        content1:
          "Receive your design within a few business days, and be updated on the process. Everything you need for a digitally driven brand. Defined proposition. Conceptual realisation. Logo, type, look, feel, tone, movement, content - we've got it covered.",
        content2:
          "Getting your brand message out there. We create dynamic campaign creative that engages audiences, wherever they are most talented. Bring your brand to life, communicate your value proposition with agile setup across creativity.",
      },
      section4: {
        heading: "Product making for friendly users",
        image: "",
        list: [
          {
            eventKey: 1,
            question: "Design should enrich our day",
            answer:
              "Our design services starts and ends with a best-in-class experience strategy that builds brands. Through a process of iteration and prototyping design interfaces that bring joy to people",
          },
          {
            eventKey: 2,
            question: "Bring their individual experience and creative",
            answer:
              "This is the second item's accordion body. It is hidden by default, until the collapse plugin adds the appropriate classes that we use to style each element. These classes control the overall appearance, as well as the showing and hiding via CSS transitions. You can modify any of this with custom CSS or overriding our default variables.",
          },
          {
            eventKey: 3,
            question: "Human centred design to challenges",
            answer:
              "Our design services starts and ends with a best-in-class experience strategy that builds brands. Through a process of iteration and prototyping design interfaces that bring joy to people",
          },
          {
            eventKey: 4,
            question: "Design should enrich our day",
            answer:
              "Our design services starts and ends with a best-in-class experience strategy that builds brands. Through a process of iteration and prototyping design interfaces that bring joy to people",
          },
          {
            eventKey: 5,
            question: "Developing core web applications",
            answer:
              "Our design services starts and ends with a best-in-class experience strategy that builds brands. Through a process of iteration and prototyping design interfaces that bring joy to people",
          },
        ],
      },
      section5: {
        text: "WORK WITH US",
        heading: "We would love to hear more about your project",
      },
    },
  },
  {
    id: 18,
    name: "ui-ux-design",
    data: {
      name: "UI/UX Design",
      section1: {
        content1:
          "Every decision we make needs to answer the million-dollar question: hozzzzzzzw will this benefit our partner’s goal best? We work to develop solutions",
        content2:
          "We like to be different, in the same way that every mission is different. Every project is an opportunity to create result that will help you",
        list: [
          "API Development",
          "Cloud Migration",
          "JavaScript",
          "WordPress",
          "Front End Development",
          "Fluter Framework",
        ],
        largeImage: "../../../public/assets/imgs/about/3/1.jpg",

        // largeImage: "../public/assets/imgs/about/3/1.jpg",
        smallImage: "",
      },
      section2: [
        {
          name: "Planning and sketching",
          discription:
            "Modern and unique design practically point of view, it risks not meeting the huge expectations",
        },
        {
          name: "Team working",
          discription:
            "Modern and unique design practically point of view, it risks not meeting the huge expectations",
        },
        {
          name: "Flowchart and wireframe",
          discription:
            "Modern and unique design practically point of view, it risks not meeting the huge expectations",
        },
        {
          name: "User experience Testing",
          discription:
            "Modern and unique design practically point of view, it risks not meeting the huge expectations",
        },
      ],
      section3: {
        heading:
          "We replace unreliable wirefreme and expensive agencies for one of the best organized layer.",
        image: "",
        content1:
          "Receive your design within a few business days, and be updated on the process. Everything you need for a digitally driven brand. Defined proposition. Conceptual realisation. Logo, type, look, feel, tone, movement, content - we've got it covered.",
        content2:
          "Getting your brand message out there. We create dynamic campaign creative that engages audiences, wherever they are most talented. Bring your brand to life, communicate your value proposition with agile setup across creativity.",
      },
      section4: {
        heading: "Product making for friendly users",
        image: "",
        list: [
          {
            eventKey: 1,
            question: "Design should enrich our day",
            answer:
              "Our design services starts and ends with a best-in-class experience strategy that builds brands. Through a process of iteration and prototyping design interfaces that bring joy to people",
          },
          {
            eventKey: 2,
            question: "Bring their individual experience and creative",
            answer:
              "This is the second item's accordion body. It is hidden by default, until the collapse plugin adds the appropriate classes that we use to style each element. These classes control the overall appearance, as well as the showing and hiding via CSS transitions. You can modify any of this with custom CSS or overriding our default variables.",
          },
          {
            eventKey: 3,
            question: "Human centred design to challenges",
            answer:
              "Our design services starts and ends with a best-in-class experience strategy that builds brands. Through a process of iteration and prototyping design interfaces that bring joy to people",
          },
          {
            eventKey: 4,
            question: "Design should enrich our day",
            answer:
              "Our design services starts and ends with a best-in-class experience strategy that builds brands. Through a process of iteration and prototyping design interfaces that bring joy to people",
          },
          {
            eventKey: 5,
            question: "Developing core web applications",
            answer:
              "Our design services starts and ends with a best-in-class experience strategy that builds brands. Through a process of iteration and prototyping design interfaces that bring joy to people",
          },
        ],
      },
      section5: {
        text: "WORK WITH US",
        heading: "We would love to hear more about your project",
      },
    },
  },
];
export default Data;
